...
# Set up a separate figure for max altitude plot
fig2, ax3 = plt.subplots(figsize=(10, 5))
line_max_altitude, = ax3.plot([], [], label='Max Altitude', color='brown')

ax3.set_title("Max Altitude Over Time")
ax3.set_xlabel("Time (s)")
ax3.set_ylabel("Max Altitude (m)")
ax3.legend()
...
def init():
    ...
    line_max_altitude.set_data([], [])
    return line_measured, line_filtered_alt, liftoff_marker, apogee_marker, line_apogee_time, line_max_altitude
...
def update(frame):
    ...
    max_altitudes_trimmed = max_altitudes[:len(time_steps)]
    ...
    line_max_altitude.set_data(time_steps[:len(max_altitudes_trimmed)], max_altitudes_trimmed)
    ...
    # Update axis limits for max altitude plot
    ax3.set_xlim(0, time_steps[:len(max_altitudes_trimmed)][-1] + 1)  # Dynamic time axis
    ax3.set_ylim(0, max(max_altitudes_trimmed) + 10)  # Dynamic max altitude
...
# ani1 = FuncAnimation(fig1, update, frames=(len(time_steps) // steps_per_frame), init_func=init, blit=True, interval=interval)
ani2 = FuncAnimation(fig2, update, frames=(len(time_steps) // steps_per_frame), init_func=init, blit=True, interval=interval)
